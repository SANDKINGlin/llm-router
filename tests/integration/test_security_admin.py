...
class TestXSSDefense:
    ...
    def test_x_content_type_options_header(self):
        """X-Content-Type-Options: nosniff (R12 SecurityHeadersMiddleware 实装)."""
        c = _client()
        r = c.get("/admin/")
        assert r.status_code == 200
        # R12 实施后 SecurityHeadersMiddleware 加 X-Content-Type-Options
        nosniff = r.headers.get("x-content-type-options", "")
        assert nosniff == "nosniff", f"Missing X-Content-Type-Options: nosniff, got: {nosniff}"
        # 顺便验 X-Frame-Options + Referrer-Policy
